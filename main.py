...
class ChannelTime(StatesGroup):
    id = State()
    timeout = State()


@dp.message(Addition.id)
async def input_report(m: types.Message, state: FSMContext):
    try:
        ...
        await bot.send_message(m.chat.id, f'❌ Текст для данного чата не был обновлен! Ошибка: {str(e)}')
...
@dp.message(ChannelTime.timeout)
async def input_report(m: types.Message, state: FSMContext):
    try:
        if int(m.text) > 1:
            data = await state.get_data()
...
async def echo_message(m: types.Message):
    ...
    if m.text == '💬 Доступные чаты':
        ...
        keyboard.add(*[types.InlineKeyboardButton(text=name, callback_data=cb) for name, cb in
                       {f'{_["title"]}': f'EDIT_ID:{_["id"]}' for _ in chats}.items()])
        ...
        db.setSpam(0)
        await bot.send_message(m.chat.id, '🗳 Отправляю последние сообщения и закругляюсь',
                               reply_markup=welcome_keyboard())
    elif m.text == '🕒 Интервал':
        settings = db.settings()
        keyboard.add(*[types.InlineKeyboardButton(text=name, callback_data=cb) for name, cb in
                       {'🕘 Изменить интервал': 'INTERVAL'}.items()])
        ...
        await bot.send_message(m.chat.id,
                               '<b>🔝 Ваш пост выглядит вот так 🔝</b>\n\n• Для поста с шрифтами используйте @format_robot',
...
async def poc_callback_but(c: types.CallbackQuery, state: FSMContext):
    ...
    if 'EDIT_ID:' in c.data:
        ...
        keyboard.add(*[types.InlineKeyboardButton(text=name, callback_data=cb) for name, cb in
                       {'❌ Покинуть чат': f'LFC:{channel_id}', '🛑 Остановить спам': f'STOP_SPAM:{channel_id}',
                        '🔄 Изменить задержку': f'CHANGE_TIMEOUT:{channel_id}'}.items()])
        if addit_text is not None:
            keyboard.add(*[types.InlineKeyboardButton(text=name, callback_data=cb) for name, cb in
                           {'🗃 Изменить дополнительный текст': f'ADD_ADDITIONAL:{channel_id}'}.items()])
            await bot.send_message(m.chat.id, f'🗃 Текущий дополнительный текст для данного чата: {addit_text}',
                                   reply_markup=keyboard)
        else:
            keyboard.add(*[types.InlineKeyboardButton(text=name, callback_data=cb) for name, cb in
                           {'🗃 Добавить дополнительный текст': f'ADD_ADDITIONAL:{channel_id}'}.items()])
            await bot.send_message(m.chat.id, f'🗃 Дополнительного текста для данного чата не найдено.',
                                   reply_markup=keyboard)
        ...
        await state.set_state(ChannelTime.timeout)
        ...
        await bot.send_message(m.chat.id, '🕒 Отправь мне интервал рассылки между чатов (в минутах):')
